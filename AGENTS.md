...
─────────────────────────────────────────────────────────────────────
Core
  src/types.ts                     284   Shared interfaces & type aliases
  src/state.ts                     280   Global state objects (typed)
  src/constants.ts                 235   Layout, colors, model options, API endpoints
  src/utils.ts                     110   SVG helpers, theme, time formatting, coord math
  src/substances.ts               1064   Substance database with pharma params

Entry & Pipeline
  src/main.ts                     1336   App entry, prompt submission, phase orchestration
  src/llm-pipeline.ts             1077   Multi-provider LLM calls with retry logic
  src/llm-response-shape.ts        184   Response normalization + stage shape validation
  src/llm-cache.ts                 145   Stage cache envelopes + cache toggles
  src/prompts.ts                   483   Prompt templates ({{placeholder}} interpolation)
  src/settings-store.ts             81   Typed local/session storage accessors
  src/dom.ts                       108   Cached DOM registry + typed element helpers
  src/task-group.ts                 44   Prompt-run cleanup for timers / RAFs

Phase Chart (barrel: src/phase-chart.ts)
  src/chart-axes.ts                816   X/Y axes, grid, level descriptors, tooltips
  src/chart-scan-lines.ts          224   Animated scan lines for chart/timeline/bio zones
  src/chart-curves.ts              370   Baseline/desired curves, morph, peak labels, legend
  src/phase-chart-ui.ts            113   Prompt errors + phase chart reset/runtime wiring
  src/curve-utils.ts               ~60   Points-to-path, smoothing, interpolation

Lx System (barrel: src/lx-system.ts)
  src/pharma-model.ts               41   substanceEffectAt() — pharma decay math
  src/lx-compute.ts                303   Intervention validation, Lx overlay computation
  src/lx-render.ts                 511   Lx curve SVG rendering, playhead, morph drag
  src/substance-timeline.ts       1316   Timeline pill layout, rendering, sequential reveal

Biometric
  src/biometric.ts                2900   Device rec, profile drafting, strip render, revision
  src/biometric-devices.ts         279   Device definitions, signal metadata, channels
  src/revision-animation.ts       1010   Revision diff: pill animations, scan, brackets
  src/svg-animate.ts                52   Generic rAF SVG animation primitives

Animation Engine
  src/timeline-engine.ts           575   Segment-based engine: play/pause/seek/events
  src/timeline-ribbon.ts           475   Canvas timeline control bar with drag-to-seek
  src/timeline-builder.ts          377   Builds segments from phase data
  src/pipeline-timeline.ts         965   LLM agent lane (API call durations)

Narration & UI
  src/sherlock.ts                  918   Card stack narration panel, Lx step controls
  src/word-cloud.ts                787   Orbital word cloud for effects
  src/baseline-editor.ts          1564   Interactive scrubber for baseline curves
  src/debug-panel.ts               454   Debug log panel with export
  src/divider.ts                   292   Split-screen divider for 2-effect mode

Timeline Segments (src/timeline-segments/)
  lx-segments.ts                   695   Lx overlay reveal animations
  curves-segments.ts               376   Baseline/desired curve animations
  sherlock-segments.ts             320   Sherlock narration card animations
  word-cloud-segments.ts           438   Word cloud entrance/dismissal
```
...
│   + Optimize / Lx / VCR / Biometric controls  │
...
The main visualization is a full-width pharmacodynamic chart (SVG 960×500) showing 24-hour effect curves, intervention overlays, timeline pills, and biometric strips. There is no parallel cartridge UI path anymore; the phase chart is the active surface.
...
- `src/main.ts`: Bootstrap, prompt flow orchestration, settings/theme wiring, timeline lifecycle
...
- `src/llm-pipeline.ts`: LLM API wrappers, provider failover, stage runners
- `src/llm-response-shape.ts`: Boundary normalization/validation for stage payloads
- `src/settings-store.ts`: Single storage boundary for persisted browser state
- `src/dom.ts`: Cached DOM registry for shared app shells/groups
...
- `src/biometric.ts`: Biometric strip flow, revision pipeline, simulation/multi-day orchestration
- `src/phase-chart.ts`: Pure barrel for chart modules
- `src/phase-chart-ui.ts`: Reset/error surface and chart runtime wiring
...
Light/dark mode toggle persisted through `src/settings-store.ts`. Chart colors dynamically computed via `chartTheme()`.
...
The phase chart + VCR + biometric flow is the only active UI path. If a module is not reachable from `src/main.ts`, treat it as dead code and remove it rather than preserving parallel legacy flows.
