...
| Stage | Agent | Module | Role |
|-------|-------|--------|------|
| 1 | Scout | INTENT | Effect identification → word cloud |
| 2 | Strategist | MAP | Pharmacodynamic landscape (baseline vs desired) |
| 3 | Chess Player | DESIGN | Protocol engineering → intervention selection |
| 4 | Spotter | LEARN | Biometric perturbation simulation → wearable strips |
| 5 | Grandmaster | LEARN | Protocol revision in response to perturbed reality |

Stages 1 & 2 run in parallel. Per-stage model selection via `AppState.stageModels`.

### 4-Agent Multi-Day Pipeline (STREAM)
| Agent | Role |
|-------|------|
| Knight | Maintains desired curves from original goal (adjusts only if week context demands) |
| Spotter Daily | Day-specific biometric perturbations (sleep debt, life events, tolerance) |
| Strategist Bio Daily | Baseline correction from biometric evidence |
| Grandmaster Daily | Per-day intervention protocol adaptation |
