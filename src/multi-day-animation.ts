...
import { svgEl, phaseChartX, phaseChartY, clamp, teleportInterpolation } from './utils';
...
function parseDoseFromZero(iv: any): DoseMorphInfo | null {
...
function parseDoseMorph(fromIv: any, toIv: any): DoseMorphInfo | null {
...
function tickPillMorph(plan: PillMorphPlan, lxEase: number, fromDay: DaySnapshot, toDay: DaySnapshot) {
    ...
    for (const { from, to, el, doseMorph, ghost } of plan.matched) {
        ...
        if (ghost) {
            ...
            // Ghost was built at `to` geometry — translate relative to its own position
            ...
            if (doseMorph && lxEase > 0.01) {
                const label = ghost.querySelector('.timeline-bar-label') as SVGTextElement | null;
                if (label) {
                    if (lxEase >= 1) {
                        // Restore plain text to match static render (no font-size pop)
                        const finalDisplay =
            ...
            // Interpolate dose label (counting number + font pulse + arrow indicator)
            if (doseMorph) {
                ...
                if (label) {
                    if (lxEase >= 1) {
                        ...
                        if (lxEase > 0.01) {
                            ...
                            arrowSpan.setAttribute('fill', arrowColor);
                            arrowSpan.setAttribute('dx', '2');
                            const arrowOpacity = Math.min(1, lxEase / 0.4);
    ...
    for (const { geo, el, doseMorph } of plan.added) {
        if (lxEase < 0.3) {
            ...
            // Set label to "Name 0unit" while invisible so first visible frame starts at 0
            if (doseMorph) {
                const label = el.querySelector('.timeline-bar-label') as SVGTextElement | null;
            ...
            if (doseMorph) {
                const label = el.querySelector('.timeline-bar-label') as SVGTextElement | null;
                if (label) {
                    const doseT = fadeIn; // dose counting tracks the fade-in progress

                    if (doseT >= 1) {
                        // Animation complete — restore plain text label (no tspans / font-size overrides)
                        // to match the static render and avoid a font-size pop on DOM rebuild
                        const finalDisplay =
                            doseMorph.decimals > 0
                                ? doseMorph.newNum.toFixed(doseMorph.decimals)
                                : String(Math.round(doseMorph.newNum));
                        ...
                        // Font-size pulse on number: 0 → +18% → 0
                        const baseFontSize = 11;
                        const pulse = Math.sin(Math.PI * doseT);
                        const pulsedSize = baseFontSize * (1 + 0.18 * pulse);
                        numSpan.setAttribute('font-size', pulsedSize.toFixed(1));

                        // Up arrow indicator — fade in during first 40%
                        if (doseT > 0.01) {
                            const arrowSpan = document.createElementNS('http://www.w3.org/2000/svg', 'tspan');
                            arrowSpan.setAttribute('fill', arrowColor);
                            arrowSpan.setAttribute('dx', '2');
...
function _interpolateFrame(
    ...
    if (pillPlan) {
        tickPillMorph(pillPlan, t, fromDay, toDay);
    }
